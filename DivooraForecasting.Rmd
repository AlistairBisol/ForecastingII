...
typeof(product_complete)
levels(product_complete$PROD_catID)
fct_count(product_complete$PROD_catID)
...
basedata$date <- as.Date(basedata$date,
                        format = "%Y/%m/%d")
class(basedata$date)
class(basedata$month)
#basedata$month <- as.Date(basedata$month,
 #                       format = "%Y/%m")

# Selection of the important variables for the time serie

dat <-tibble(basedata$driver_id,basedata$date,basedata$month)

# number of daily delivery per driver 
DDPD = dat %>%
  count(basedata$driver_id,basedata$date)

colnames(DDPD)[2] <- "date"

# number of delivery per day
 
dailydelivery = DDPD %>%
  group_by(date) %>%
  summarize(totperday = sum(n))


# Plot of the number of delivery per day through time from the 10th of December 2019 to the 1st of January 2022

test = ggplot(data = dailydelivery, aes(x = dailydelivery$date, y = dailydelivery$totperday)) +
  geom_point() +
  labs(x = "Date",
    y = "number of delivery per day",
    title = "Delivery through time",
    subtitle = "Driver")
test
...
#cumulative delivery
#week day
#number of delivery per day of the week, 
#evolution through time with barplot, 
# transform in order to have the day of the week for each day
dailydelivery <- tibble(dailydelivery,1:nrow(dailydelivery))
dailydelivery[3] = weekdays(dailydelivery$date)
colnames(dailydelivery)[3]="Weekday"

#have the total of delivery per day
totperweeekday = dailydelivery %>%
  group_by(Weekday) %>%
  summarize(totperweekday = sum(totperday))

# the barplot of total per weekday:
barplot(height=totperweeekday$totperweekday, names=totperweeekday$Weekday ,
        density=c(5,10,20,30,7,15,24) , angle=11 ,
        col=rgb(0.8,0.1,0.1,0.6),
        xlab="Day of the week", 
        ylab="Number of delivery", 
        main="Distribution of the delivery through the day of the week") 


# Nice graph example where each line would correspond to a day of the week

wewew <- ggplot(
  dailydelivery, 
  aes(x = dailydelivery$totperday, y = dailydelivery$Weekday)
  ) +
  ...
  scale_fill_gradientn(
    ...
    name = "Dailydelivery number"
    )+
  labs(title = 'Distribution of the number of delivery') 
wewew

# number of delivery per month

DPM = dat %>%
  count(basedata$driver_id,basedata$month)

colnames(DPM)[2] <- "month"

monthdelivery = DPM %>%
  group_by(month) %>%
  summarize(totpermonth = sum(n))


# Plot of the number of delivery per month through time from december 2019 to january 2022

test = ggplot(data = monthdelivery, aes(x = monthdelivery$month, y = monthdelivery$totpermonth)) +
  geom_point() +
  labs(x = "Date",
    y = "number of delivery per month",
    title = "Delivery through time",
    subtitle = "Driver")
test
...
## Add Covid to delivery Time Serie

```{r, echo = F,warning=F, message = F}

DailyCovidCase$date <- as.Date(DailyCovidCase$date,
                        format = "%Y/%m/%d")
class(DailyCovidCase$date)

test2 = ggplot(data = DailyCovidCase, aes(x = DailyCovidCase$date, y = DailyCovidCase$ncumul_hosp)) +
  geom_point() +
  labs(x = "Date",
    y = "number of hosp per day",
    title = "Covid case through time",
    subtitle = "Driver")
test2

MergeDeliveryCovid <- full_join(DailyCovidCase, dailydelivery, by = "date")

test2 = ggplot(data = MergeDeliveryCovid, aes(x = MergeDeliveryCovid$date), color=y) +
  geom_point(aes(y=MergeDeliveryCovid$ncumul_hosp),color=2) + geom_point(aes(y=MergeDeliveryCovid$totperday)) +
  labs(x = "Date",
    y = "Number of delivery (black) and covid (red)",
    title = "Delivery & Covid through time",
    subtitle = "Driver")
test2
```